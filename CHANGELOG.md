...
### Removed (breaking)
- **PNG / PDF chart export.** Removed `ChartExport`, `ChartExporter`, `PngChartExporter`,
  `PdfChartExporter`, and `pdfAsciiText` (`core/canvas/chart_exporter.dart`).
- **`ChartWidgetController`** and **`ChartCard`** are gone, along with the `controller`
  parameter on every chart widget and on `ChartWidget`.
- Dropped the `pdf` package dependency — the library is now pure Flutter SDK with
  zero third-party runtime dependencies.

### Migration
- Remove any `controller:` arguments passed to chart widgets.
- Replace `ChartCard(...)` with the chart widget directly.
