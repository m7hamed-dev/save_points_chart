...
class StackedColumnChartPainter extends BaseChartPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxY = totalsByX.values.isEmpty ? 1.0 : totalsByX.values.reduce(math.max) * 1.15;
    ...
    for (int xIndex = 0; xIndex < sortedXValues.length; xIndex++) {
      ...
      final xProgress = math.max(0.0, math.min(1.0, (animationProgress - (xIndex / sortedXValues.length) * 0.3) / 0.7));
      ...
      for (int dataSetIndex = 0; dataSetIndex < dataSets.length; dataSetIndex++) {
        ...
        if (!yHeight.isFinite || !yStart.isFinite || !yEnd.isFinite || yHeight <= 0 || yStart < yEnd) {
          continue;
        }
        ...
        final barRect = RRect.fromRectAndRadius(
          Rect.fromLTRB(xPos - barWidth / 2, yEnd, xPos + barWidth / 2, yStart),
        ...
        final gradient = LinearGradient(
          ...
          colors: [dataSet.color, dataSet.color.withValues(alpha: 0.7)],
        );
        ...
        final borderPaint = Paint()
          ..color = isSelected ? Colors.white : dataSet.color.withValues(alpha: 0.3)
    ...
    canvas.save();
    canvas.translate(chartOffset.dx, chartOffset.dy);
    drawAxisLabels(canvas, chartSize, minX - xPadding, maxX + xPadding, minY, maxY, dataSets: dataSets);
    canvas.restore();
  }
